...
class TestpostItem(BaseTestCase):
    ...
    def test_post_id_is_invalid_in_request(self):
        """
        Test that the post Id is invalid
        :return:
        """
        with self.client:
            response = self.client.post(
                'v1/postlists/id/items/',
                data=json.dumps(dict(name='Food')),
            ...
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'failed')
            self.assertTrue(data['message'] == 'Provide a valid post Id')
            self.assertEqual(response.status_code, 401)

    def test_post_id_is_invalid_in_put_request(self):
        """
        Test that the post Id is invalid
        :return:
        """
    ...
    def test_correct_response_when_user_has_no_post_with_specified_id_post_request(self):
        """
        Test that a user does not have a post specified by that Id
        :return:
        """
        with self.client:
            response = self.client.post(
            ...
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'failed')
            self.assertTrue(data['message'] == 'User has no post with Id 1')
            self.assertEqual(response.status_code, 202)

    def test_correct_response_when_user_has_no_post_with_specified_id_get_request(self):
        """
        Test that a user does not have a post specified by that Id
        :return:
        """
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'failed')
            self.assertTrue(data['message'] == 'post not found')
            self.assertEqual(response.status_code, 404)

    def test_an_item_has_been_successfully_saved(self):
        """
        Test a post Item has been successfully stored.
        :return:
        """
        with self.client:
            token = self.get_user_token()
    ...
    def test_items_are_returned(self):
        """
        Test post Items are returned and the items are in a list
        :return:
        """
        with self.client:
            token = self.get_user_token()
    ...
    def test_items_returned_when_searched(self):
        """
        Test post Items are returned when a query search q is present in the url
        Also test that the next page pagination string is 'http://localhostv1/postlists/1/items/?page=2'
        ...
        with self.client:
            ...
            self.assertEqual(data['items'][0]['postId'], 1)
            self.assertEqual(data['items'][0]['id'], 6)
            self.assertEqual(data['count'], 6)
            self.assertEqual(data['next'], 'http://localhost/v1/postlists/1/items/?q=f&page=2')
            self.assertEqual(data['previous'], None)
            self.assertEqual(response.status_code, 200)
    ...
    def test_items_returned_when_searched_2(self):
        """
        Test post Items are returned when a query search q is present in the url
        Also test that the next page pagination is none and previous 'http://localhostv1/postlists/1/items/?page=1'
        :return:
        """
        with self.client:
            token = self.get_user_token()
            ...
            self.create_items(token)
            response = self.client.get(
                'v1/postlists/1/items/?q=f&page=2',
                headers=dict(Authorization='Bearer ' + token)
            )
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'success')
            ...
            self.assertEqual(data['count'], 6)
            self.assertEqual(data['next'], None)
            self.assertEqual(data['previous'], 'http://localhost/v1/postlists/1/items/?q=f&page=1')
            self.assertEqual(response.status_code, 200)

    def test_empty_item_list_is_returned_when_no_items_in_post(self):
        """
        Test empty items list is returned when the post is empty
        :return:
        """
        with self.client:
            token = self.get_user_token()
    ...
    def test_invalid_item_id_get_one_item_request(self):
        ...
        with self.client:
            response = self.client.get(
                'v1/postlists/1/items/dsfdgfghjg/',
                headers=dict(Authorization='Bearer ' + self.get_user_token())
            )
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'failed')
    ...
    def test_no_post_on_get_item_request(self):
        """
        Test there is no post specified by that Id when getting an item by Id
        :return:
        """
        with self.client:
            response = self.client.get(
    ...
    def test_no_post_delete_request(self):
        """
        Test there is no post specified by that Id
        :return:
        """
        with self.client:
            response = self.client.delete(
    ...
    def test_item_is_deleted_successfully(self):
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'success')
            self.assertTrue(data['message'] == 'Successfully deleted the item from post with Id 1')
            self.assertEqual(response.status_code, 200)

    def create_item(self, token):
        """
        Create an item into a post
        :param token:
        :return:
        """
        response = self.client.post(
